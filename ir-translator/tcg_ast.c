...
void translate_qemu_ld(OpCodeType opc, void *ptr) {
    ...
    LLVMValueRef result = build_load_with_alignment(builder, llvm_int_types[type_mem], pointer, get_next_var_name(opcode_type_str[opc], operand0), align);
...
void translate_st(OpCodeType opc, void *ptr) {
    ...
    if ((operand1.s.slot_type == SUB_SLOT_TMP && has_alias_xmm(operand1)) || operand1.s.slot_type == SUB_SLOT_XMM) {
        ...
        check_scalable_vector_perform_store(val, type_mem, addr_val, GET_ALIGNMENT_FROM_CONSTANT(operand1.s.offset));
    }
}
...
static void translate_short_circuit_jmp_ind(OpCodeType opc, void *ptr) {
    ...
    if (is_imm[operands_cnt-2] && is_imm[operands_cnt-1]) {
        assert(operands[operands_cnt-2].i < operands[operands_cnt-1].i && operands[operands_cnt-2].i == 0 && (operands[operands_cnt-1].i == 1 || (operands[operands_cnt-1].i % SHADOW_JUMP_TABLE_BLOCK_SIZE) == 0) && operands[operands_cnt-1].i <= (7 * SHADOW_JUMP_TABLE_BLOCK_SIZE));
