...
while (<FD>) {
  ...
  if ($line =~ /^<FunctionDefinition/) {
    ...
    if ($info{'NAME'} =~ /^helper_/ and $info{'NAME'} =~ /_ymm$/) {
...
foreach my $f (keys %covered_funcs) {
  ...
  if ($funcs{$f}->{'NAME'} =~ /^helper_/ and $funcs{$f}->{'NAME'} =~ /_ymm$/ and $funcs{$f}->{'FUNC_TYPE'} ne "void") {
    ...
    if ($func_idx != -1) {
      if (exists $func_ptr->{'TOUCHED'}) {
        if ($file_content[$stop+1] ne "-") {
          ...
          if ($aot_level == 3 and (not ($sym_info[0]->{'SYM'} eq "cc_src" or $sym_info[0]->{'SYM'} eq "cc_dst" or $sym_info[0]->{'SYM'} eq "cc_op" or $sym_info[0]->{'SYM'} eq "regs" or $sym_info[0]->{'SYM'} eq "xmm_regs"))) {
...
sub parse_func_head
{
  ...
  $func_type_info =~ s/^\s*//;
  my $func_type = "NA";
  if ($func->{'NAME'} =~ /^helper_/ and $func->{'NAME'} =~ /_ymm$/) {
  ...
  if (not ($func->{'NAME'} =~ /^helper_/ and $func->{'NAME'} =~ /_ymm$/)) {
  ...
  foreach my $e (@sorted_events) {
    ...
    if (exists $func_ptr->{'CALLS'}->{$e}) {
      ...
      if ($vec_entry->{'TYPE'} eq "VecQ") {
        ...
        $body = $body.$sub_str;
        $body = $body."\n$vec_var = (v4ulong)vec_assign_tmp;\n";
        $body = $body."}\n";
