...
static void gen_tcg_ir(CPUState *cpu)
{
    ...
    uint64_t cs_base = 0;
#if 1
    int cflags = 0xff0383ff/*cflags: CF_CLUSTER_MASK | CF_PCREL | CF_NOIRQ | CF_NO_GOTO_TB | CF_PARALLEL*/;
#else
    int cflags = 0xff038201/*cflags: CF_CLUSTER_MASK | CF_PCREL | CF_NOIRQ | CF_NO_GOTO_TB | one-shot TB with 1 insn in it | CF_PARALLEL*/;
#endif
